...
(() => {
  ...
  // src/gym-med/blurInElements.ts
  (function() {
    const blurProperty = gsapWithCSS.utils.checkPrefix("filter"), blurExp = /blur\((.+)?px\)/, getBlurMatch = (target) => (gsapWithCSS.getProperty(target, blurProperty) || "").match(blurExp) || [];
    gsapWithCSS.registerPlugin({
      ...
      init(target, endValue) {
        let data = this, filter = gsapWithCSS.getProperty(target, blurProperty), endBlur = "blur(" + endValue + "px)", match = getBlurMatch(target)[0], index;
        if (filter === "none") {
          filter = "";
        }
        if (match) {
        ...
        data.target = target;
        data.interp = gsapWithCSS.utils.interpolate(filter, endValue);
      },
  ...
  var blurInElements = () => {
    ...
    elements.forEach((element) => {
      ...
      let split2 = new SplitType(element, {
